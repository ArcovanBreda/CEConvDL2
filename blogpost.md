...
However, a small error is present here as the sum $\sum^{C^l}_{c=1}$ indicates that $f_c(y)$ and $\psi_c^i(y - x)$ are scalar values. This interpretation is inconsistent given the dot product and the matrix $H_n(k)$.
