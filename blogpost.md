...
$$
with $f(x)_{h,s,v}$ indicating the respecitve hue, saturation or value at pixel $x$ in the input image $f$. Because the Hue value is defined on a cyclic interval the dot product between a hue shifted image ($f$) is the same as the dot product between an inverse hue shifted filter ($\psi$), in contrast to the reproduced paper in which they model hue shifts as a 3D rotation meaning they can fall out of the RGB cube:
$$
